...
def specificity_score(fact: str) -> float:
...
def main(dataset_path):
    ...
    for path, folder_name, file_name, text in tqdm.tqdm(load_text_files(dataset_path)):
        ...
        extracted_facts = extract_facts_dict(response, folder_name)
        print(f"EXTRACTED FACTS IN LOOP: {extracted_facts}")
        all_facts.append(extracted_facts)
    ...
    print(f"\nSaved facts to {output_file}")
    #TODO add the same facts to usual vs, and evaluate the speed of answer
    # performance metrics
    performance_metrics = get_performance_metrics(
        total_elapsed, processing_times, processed_articles, n_facts)
    # context separation metrics
    flat_facts = [fact for sub in all_facts for fact in sub]
    context_separation_metrics = get_context_separation_metrics(flat_facts)
    # importance metrics
    importance_metrics = get_facts_importance_metrics(flat_facts) #it's just easier to me to pass the facts like this, cause I already creted the functions, in the future, I will take them from the vs
    # redundancy metrics
