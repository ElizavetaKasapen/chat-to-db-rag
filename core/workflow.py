...
def _update_existing_context(context_id: str, facts: list[str]) -> str:
    ...
    for fact in facts:
        ...
        # Prepare request for Memory Manager #TODO templates?
        request = {
            "messages": [
                HumanMessage(
...
def _create_new_context(facts: list[str], representation: str) -> str:
    ...
    facts_texts = "\n\n" + "\n\n".join(fact["text"] for fact in facts_info)

    return f"Created context '{context_id}' with {len(facts_info)} facts.\n\nStored facts: {facts_texts}\n"
...
def _process_factual_information(user_request: str) -> str:
    ...
    similar_context = context_similarity_search_tool.run({
        "query": input_context_text, "add_facts_mode": True
    })
